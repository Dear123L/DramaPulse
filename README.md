...
| GET | `/episodes` | 获取所有剧集 |
| GET | `/episodes/{no}` | 获取单集信息 |
| GET | `/episodes/{no}/highlights` | 获取某集高光点（**App 核心接口**） |
| POST | `/ai/branch` | 【方案A】AI 文字续写 |
| POST | `/ai/branch-with-image` | 【方案B】文字+文生图 |
| POST | `/ai/branch-video` | 【方案C】获取预渲染视频 |
| GET | `/ai/branch-video/{ep}/{hl}/{branch}` | 直接返回分支视频文件 |
| POST | `/analyze` | 触发视频分析（同步） |
